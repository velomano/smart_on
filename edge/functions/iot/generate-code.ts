// IoT 코드 생성 API
import { sensors, controls } from 'iot-templates';
...
export default function generateCode(req: GenerateCodeRequest): string {
  ...
  const controlWrite = new Set<string>();
  ...
  controlSpecs.forEach(({ type, count }) => {
    ...
    switch (type) {
      case 'relay':
        ...
        controlWrite.add('digitalWrite(RELAY_PIN, state);');
        break;
      case 'dc_fan_pwm':
        controlDefines.add('#define FAN_PIN 18');
        controlInit.add('pinMode(FAN_PIN, OUTPUT);');
        controlWrite.add('analogWrite(FAN_PIN, speed);');
        break;
      case 'servo':
        ...
        controlWrite.add('servo.write(angle);');
        break;
      case 'led_strip':
        controlDefines.add('#define LED_PIN 20');
        controlInit.add('pinMode(LED_PIN, OUTPUT);');
        controlWrite.add('analogWrite(LED_PIN, brightness);');
        break;
    }
  });
